...
from research_analytics_suite.hardware_manager.cpu.CPUManager import CPUManager
...
class TestCPUInstaller:
    ...
    def cpu_installer(self, logger, remote_manager):
        return CPUManager(logger, remote_manager)
    ...
    def test_initialization_with_remote_servers(self, logger, remote_manager):
        ...
        cpu_installer = CPUManager(logger, remote_manager, remote_servers)
        assert cpu_installer.logger == logger
        assert cpu_installer.remote_manager == remote_manager
        assert isinstance(cpu_installer.cpu_detector, CPUDetector)
    ...
    def test_install_no_cpus_detected(self, cpu_installer, logger):
        with mock.patch.object(cpu_installer.cpu_detector, 'detect_cpu', return_value=[]):
            result = cpu_installer.detect_hardware()
            assert result == {}
    ...
    def test_install_with_cpus_detected(self, cpu_installer, logger):
        ...
        with mock.patch.object(cpu_installer.cpu_detector, 'detect_cpu', return_value=detected_cpus):
            result = cpu_installer.detect_hardware()
    ...
    def test_install_exception_during_cpu_detection(self, cpu_installer, logger):
        with mock.patch.object(cpu_installer.cpu_detector, 'detect_cpu', side_effect=Exception("Detection error")):
            result = cpu_installer.detect_hardware()
            assert result == {}
