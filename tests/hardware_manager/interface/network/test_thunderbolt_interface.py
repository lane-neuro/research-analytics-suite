...
from research_analytics_suite.hardware_manager.interface.network.ThunderboltInterface import ThunderboltInterface
...
def thunderbolt_interface(mock_platform_system, logger):
    return ThunderboltInterface(logger)


class TestThunderboltInterface:
    @patch('platform.system', return_value='Linux')
    def test_detect_os_linux(self, mock_platform_system, logger):
        interface = ThunderboltInterface(logger)
