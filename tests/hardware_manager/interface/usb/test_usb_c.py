...
from research_analytics_suite.hardware_manager.interface.usb.USBc import USBc
...
def usbc_interface(mock_platform_system, logger):
    return USBc(logger)
...
class TestUSBc:
    ...
    def test_detect_success(self, mock_platform_system, mock_subprocess_run, logger):
        mock_subprocess_run.return_value = MagicMock(stdout='Bus 001 Device 002: ID 8087:0024 Intel Corp. USB-C Device', stderr='')
        interface = USBc(logger)
        devices = interface.detect()
        assert devices == [{'bus': '001', 'device': '002', 'vendor_id': '8087', 'product_id': '0024', 'description': 'Intel Corp. USB-C Device'}]
        interface.logger.debug.assert_any_call('Executing command: lsusb')
        interface.logger.debug.assert_any_call('Command output: Bus 001 Device 002: ID 8087:0024 Intel Corp. USB-C Device')
    ...
    def test_parse_output_linux(self, logger):
        ...
        output = "Bus 001 Device 002: ID 8087:0024 Intel Corp. USB-C Device\nBus 002 Device 003: ID 1234:5678 Test USB-C Device"
        parsed = interface._parse_output(output)
        assert parsed == [
            {'bus': '001', 'device': '002', 'vendor_id': '8087', 'product_id': '0024', 'description': 'Intel Corp. USB-C Device'},
            {'bus': '002', 'device': '003', 'vendor_id': '1234', 'product_id': '5678', 'description': 'Test USB-C Device'}
        ]
    ...
    def test_parse_output_windows(self, logger):
        ...
        output = "Status     Class           FriendlyName        InstanceId\n" \
                 "------     -----           ------------        ----------\n" \
                 "OK         USB             USB-C Device        USB\\VID_1234&PID_5678\\12345678"
        parsed = interface._parse_output(output)
        assert parsed == [
            {'status': 'OK', 'class': 'USB', 'friendly_name': 'USB-C Device', 'instance_id': 'USB\\VID_1234&PID_5678\\12345678'}
        ]
