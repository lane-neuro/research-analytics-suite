...
<div align="center">
  ...
  <a href="https://github.com/lane-neuro/research-analytics-suite">
    <img src="../../../gui/assets/images/centered_banner_white_black_text_1800x700.png" alt="Research Analytics Suite Banner" style="max-width: 75%; height: auto;">
  </a>
  <p>Author: <a href="#contact">Lane</a></p>
</div>

# /operation_manager/operations/persistent
### Part of Research Analytics Suite (RAS)
<div>
  <p align="left">
The `operation_manager/operations/persistent` subpackage handles persistent operations within the RAS.
<br />
<br />
...
The `persistent` subpackage within the `operation_manager/operations` module defines and manages persistent operations in the Research Analytics Suite (RAS). The key components of this subpackage include:

- **ConsoleOperation**: Handles user-input functions from the console, providing methods for displaying a prompt and processing input.
- **ResourceMonitorOperation**: Continuously monitors the usage of CPU and memory, handling errors if usage exceeds specified thresholds.

### ConsoleOperation
The `ConsoleOperation` class is designed to handle user-input functions from the console. It provides methods for displaying a prompt for user input and processing the input data.

### ResourceMonitorOperation
The `ResourceMonitorOperation` class monitors the usage of CPU and memory, ensuring that usage does not exceed specified thresholds. It continuously checks resource usage and handles errors if the usage exceeds these thresholds.
...
Usage instructions for the Persistent subpackage will be provided at a later date.
